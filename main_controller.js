// ===================================================================================
// ### main_controller.js (V5.3 - 增加“邮箱超时”豁免重试规则) ###
// ===================================================================================
            ...
            if (activeWorkflowKey) {
                ...
                while (moduleRetries < MAX_MODULE_RETRIES) {
                    try {
                        ...
                        // --- 新增逻辑: 检查是否为“邮箱API超时”的特殊失败 ---
                        if (error.message.includes("EMAIL_API_TIMEOUT")) {
                            console.error(`[${instanceId} 最终失败] 接到模块3的特殊失败信号，此账户将被终止，不进行重试。`);
                            throw error; // 直接将此特殊错误上抛，终止整个工作流
                        }
                        // ----------------------------------------------------
